...
function getCombinedCollectionWithIndices(AOI: any, endDate: any, cloudCover: number, cloudCoverUser: number, indices: string) {
	let combinedCollection = getCombinedCollection(AOI, endDate, cloudCover, cloudCoverUser)

	console.log(JSON.parse(indices));
		...
		if (val) {
			switch (idx) {
				case IndicesCode.NDVI:
					combinedCollection = combinedCollection.map(addNDVI)
					break
				case IndicesCode.mNDWI:
					combinedCollection = combinedCollection.map(addmNDWI)
					break
				case IndicesCode.AWEIsh:
					combinedCollection = combinedCollection.map(addAWEIsh)
					break
				case IndicesCode.AWEInsh:
					combinedCollection = combinedCollection.map(addAWEInsh)
					break
				case IndicesCode.NDBI:
					combinedCollection = combinedCollection.map(addNDBI)
					break
				case IndicesCode.BSI:
					combinedCollection = combinedCollection.map(addBSI)
					break
			}
		}
	})
	return combinedCollection.sort('system:time_start')
}

function getUniqueDatesCollection(AOI: any, endDate: any, cloudCover: number, cloudCoverUser: number, indices: string) {
	return getCombinedCollectionWithIndices(AOI, endDate, cloudCover, cloudCoverUser, indices).aggregate_array('system:time_start')
		.map(function(date: any) {
			return ee.Date(date).format('YYYY-MM-dd');
		}).distinct()
}

export function getDailyMedianCollection(AOI: any, endDate: any, cloudCover: number, cloudCoverUser: number, indices: string) {
	return ee.ImageCollection.fromImages(
		getUniqueDatesCollection(AOI, endDate, cloudCover, cloudCoverUser, indices).map(function(date: any) {
			date = ee.String(date); // Ensure date is a string
			let filtered = getCombinedCollectionWithIndices(AOI, endDate, cloudCover, cloudCoverUser, indices).filter(ee.Filter.date(date, ee.Date(date).advance(1, 'day')));
...
function dailyMedianWithDifferenceStart(AOI: any, startDateUser: string, endDate: any, cloudCover: number, cloudCoverUser: number, indices: string) {
	return getDailyMedianCollection(AOI, endDate, cloudCover, cloudCoverUser, indices).map(function(image: any) {
		var diff = image.date().difference(startDateUser, 'day').abs();
		return image.set('date_difference', diff);
	})
}

function dailyMedianWithDifferenceStop(AOI: any, endDateUser: string, endDate: any, cloudCover: number, cloudCoverUser: number, indices: string) {
	return getDailyMedianCollection(AOI, endDate, cloudCover, cloudCoverUser, indices).map(function(image: any) {
...
export function getStartImage(AOI: any, startDateUser: string, endDate: any, cloudCover: number, cloudCoverUser: number, indices: string) {
	return dailyMedianWithDifferenceStart(AOI, startDateUser, endDate, cloudCover, cloudCoverUser, indices).sort('date_difference').first()
}

export function getRecentImage(AOI: any, endDateUser: string, endDate: any, cloudCover: number, cloudCoverUser: number, indices: string) {
	return dailyMedianWithDifferenceStop(AOI, endDateUser, endDate, cloudCover, cloudCoverUser, indices).sort('date_difference').first()
}
